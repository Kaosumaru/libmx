...
{
public:
    KeyAction(ControlScheme* scheme, int keycode)
        : Action(scheme)
    {
...
{
public:
    JoyButtonAction(ControlScheme* scheme, unsigned joy, unsigned button)
        : Action(scheme)
    {
...
{
public:
    JoyAxisAction(ControlScheme* scheme, unsigned joy, unsigned axis, float margin)
        : Action(scheme)
    {
        auto& joysticks = MX::Window::current().joysticks()->joysticks();
        if (joy >= joysticks.size())
