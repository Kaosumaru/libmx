...
namespace Game
{
    ...
    {
    public:
        ActionBase(ControlScheme* scheme)
            : _scheme(scheme)
        {
            if (scheme)
    ...
    public:
        ...
        void bind(const pointer& proxy)
        {
            _children.push_back(proxy);
            onAddedChild(proxy);
        }
    ...
    protected:
        ...
        void OnTimeout(float tick)
        {
            ...
            if (tick && _scheme)
                _scheme->queue().planWeakFunctor(
                    tick, [&] { OnTimeout(_tickRate); }, _conn);
        }
    ...
    {
    public:
        TickingAction(ControlScheme* scheme, float tickRate = 0.0f)
            : Action(scheme)
            , TickingHelper(scheme, tickRate)
        {
    ...
    {
    public:
        ActionList(ControlScheme* scheme)
            : ActionBase(scheme)
        {
            unsigned index = 0;
    ...
    {
    public:
        TickingTargetDirection(ControlScheme* scheme, float tickRate = 0.0f)
            : TargetDirection<Type>(scheme)
            , TickingHelper(scheme, tickRate)
        {
