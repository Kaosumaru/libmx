...
{
public:
    ft_html_container_base(const HTMLRendererQueue::Options& options = {})
        : _options(options)
    {
    }
...
public:
    ...
    {
        if (_options.default_size != 0.0f)
            return _options.default_size;
...
{
public:
    ft_html_container(const HTMLRendererQueue::Options& options)
        : ft_html_container_base(options)
    {
    }
};
}
