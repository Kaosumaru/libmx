...
{
public:
    ImageSpriteAdapter(const std::shared_ptr<Graphic::Image>& image = nullptr)
        : _image(image)
    {
    }

    template <typename X>
    ImageSpriteAdapter(X& t, const std::shared_ptr<Graphic::Image>& image)
        : _image(image)
        , T(t)
    {
    }
...
{
public:
    AnimatedSpriteAdapter(const Graphic::Animation::pointer& animation = nullptr)
        : _animation(animation)
    {
    }

    template <typename X>
    AnimatedSpriteAdapter(X& t, const Graphic::Animation::pointer& animation = nullptr)
        : _animation(animation)
        , T(t)
    {
    }
...
{
public:
    SingleRunAnimatedSpriteAdapter(const std::shared_ptr<Graphic::Animation>& animation = nullptr, bool freeze_last_frame = false)
        : _animation(animation)
    {
    ...
    template <typename X>
    SingleRunAnimatedSpriteAdapter(X& t, const std::shared_ptr<Graphic::Animation>& animation = nullptr, bool freeze_last_frame = false)
        : _animation(animation)
        , T(t)
    {
        _freeze_last_frame = freeze_last_frame;
