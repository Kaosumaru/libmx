...
WaitCommand::WaitCommand(const WaitCommand& other)
...
{
public:
    QueueCommand(const QueueCommand& other)
        : CompositeCommand(other)
    {
    }
    QueueCommand(LScriptObject& script)
        : CompositeCommand(script)
    {
    }
    QueueCommand(list<Command::pointer>&& c)
        : CompositeCommand(std::move(c))
    {
    }
...
{
public:
    LoopCommand(const LoopCommand& other)
        : _iterations(other._iterations)
        , CompositeCommand(other)
    {
    }
    LoopCommand(LScriptObject& script)
        : CompositeCommand(script)
    {
        script.load_property(_iterations, "Iterations");
    }
    LoopCommand(list<Command::pointer>&& c, int i)
        : _iterations(i + 1)
        , CompositeCommand(std::move(c))
    {
    }
    LoopCommand(list<Command::pointer>&& c)
        : CompositeCommand(std::move(c))
    {
    }
...
{
public:
    SimultaneusCommand(const SimultaneusCommand& other)
        : CompositeCommand(other)
    {
    }
    SimultaneusCommand(LScriptObject& script)
        : CompositeCommand(script)
    {
    }
    SimultaneusCommand(list<Command::pointer>&& c)
        : CompositeCommand(std::move(c))
    {
    }
...
public:
    ...
    LambdaCommand(const Function& t)
        : _t(t)
    {
    }
