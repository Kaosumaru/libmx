...
Slice9Image::Slice9Image(const MX::Margins& margins, const Graphic::TextureImage::pointer& image)
    : QuadImageHolder(image)
{
    _margins = margins;
...
Slice9Image::Slice9Image(const std::vector<ImagePtr>& images)
    : QuadImageHolder(nullptr)
{
    _tiled = true;
    if (images.size() < 9)
        return;
