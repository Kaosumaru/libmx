...
public:
	...
	public:
		...
		ContextGuard(T& _target) : _old(&(ScopeSingleton<T, T1>::current())), _hasValue(true) { Reset(_target); }
		~ContextGuard() { if (_hasValue) ScopeSingleton<T, T1>::SetCurrent(*_old); }
...
template <class T, typename T1> T* ScopeSingleton<T, T1> ::_current_static_singleton = nullptr;
...
public:
	...
	{
	public:
		Context(T& _target) : _old(ScopeSingletonDefault<T>::current()) { ScopeSingletonDefault<T>::SetCurrent(_target); }
...
template <class T> T* ScopeSingletonDefault<T> ::_current_static_singleton = nullptr;
...
public:
	...
	{
	public:
		Context(T& _target) : _old(T::current()) { T::SetCurrent(_target); }
...
template <class T> T* ScopeSingletonFunction<T> ::_current_static_singleton = nullptr;
