...
{
public:
	MSLScriptParser(const std::string &path, Script &script, bool is_data = false) : ScriptParser(script), _file_path(path), _is_data(is_data)
	{
...
public:
	JSONScriptParser(char *buffer, Script &script, const char *fileName) : ScriptParser(script), _buffer(buffer)
	{
		_fileName = fileName;
	}
	bool Parse()
...
ScriptParser::ScriptParser(Script &Script) : _Script(Script)
{
