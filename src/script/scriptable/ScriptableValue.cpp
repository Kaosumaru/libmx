...
{
public:
    StringValue(Scriptable::Value* parentValue, const std::wstring& value)
...
{
public:
    FunctorValue(const std::function<float()>& number_functor)
        : _number_functor(number_functor)
    {
    }
...
{
public:
    RPNValue(Scriptable::Value* parentValue, std::string&& txt)
        : _parentValue(parentValue)
        , _text(std::move(txt))
    {
        if (isOnMainThread)
...
{
public:
    ArrayValue(const std::vector<Scriptable::Value::pointer>& members)
        : _members(members)
    {
    }
...
{
public:
    ObjectValue(const std::string& fullPath, const Map& map)
        : _fullPath(fullPath)
        , _members(map)
    {
    }
    ObjectValue(const std::string& fullPath, const std::shared_ptr<ScriptObject>& object)
        : _fullPath(fullPath)
        , _object(object)
    {
    }
...
Scriptable::Value::Value(const std::string& fullPath)
    : _fullPath(fullPath)
{
