...
ScriptString::ScriptString(const char* label)
    : _label(label)
{
}

ScriptString::ScriptString(const std::string& label)
    : _data(label)
    , _label(_data.c_str())
{
}
...
ScriptString::ScriptString(std::string&& label)
    : _data(std::move(label))
    , _label(_data.c_str())
{
}
...
#ifdef WIPSERIALIZE
ScriptSettings::ScriptSettings(const std::string& objectName, const std::string& prefix)
    : ScriptObjectString(prefix + objectName)
{
}
