...
namespace Widgets
{
    ...
    public:
        ...
        void SetFont(const Graphic::Font::pointer& font)
        {
            if (_font == font && !_bitmapFont)
                return;
            _bitmapFont = nullptr;
            _dirty = true;
            _font = font;
        }
        void SetBitmapFont(const Graphic::BitmapFontSized::pointer& font)
        {
            if (_bitmapFont == font && !_font)
                return;
            _font = nullptr;
            _dirty = true;
            _bitmapFont = font;
        ...
        const std::shared_ptr<Graphic::Image>& textImage()
        {
            UpdateTextImage();
            return _textImage;
        }
        const Graphic::RenderQueue& renderQueue()
        {
            UpdateTextImage();
            return _renderQueue;
        }
    ...
    protected:
        void SetTextImage(const std::shared_ptr<Graphic::Image>& image)
        {
            _textImage = image;
            _dirty = false;
        }
    ...
    namespace Strategy
    {
        namespace Drawable
        {
            ...
            public:
                ...
                void SetStringBuilder(const StringBuilder& builder)
                {
                    _stringBuilder = builder;
                    Build();
                }
